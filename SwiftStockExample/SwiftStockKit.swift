...
class SwiftStockKit {
    ...
    class func fetchNewsfromSymBol(url: String, completion:@escaping (_ newsArray: [News]) -> ()) {
        DispatchQueue.global().async {
            ...
            Alamofire.request(url).responseData { response in
                ...
                for item in xml["rss"]["channel"]["item"].all{
                    let news = News(title: (item["title"].element?.text)!, description: (item["description"].element?.text)!, link: (item["link"].element?.text)!, date: (item["pubDate"].element?.text)!
                    )
...
class SwiftStockChart: UIView {
    ...
    func timeLabelsForTimeFrame(_ range: ChartTimeRange) -> [String] {
        ...
        case .fiveDays:
            ...
            let weekday = Int((Calendar(identifier: Calendar.Identifier.gregorian) as NSCalendar).components(.weekday, from: Date()).weekday!)
            switch weekday {
            case 1:
